...
public partial class BitmaskSolver
{
 ...
 private void RunUniqueUnified() => RunUniqueInternal(parallel: UseParallel);
 ...
 // Expand small-N fundamental count if engine enumerated half roots only
 if (N <=8)
 {
 // For odd N, center counted once; for even N no center.
 // Engine fundamental count includes non-center roots plus center if odd.
 if ((N &1) ==1)
 {
 // Remove center contribution then double non-center, then add center back
 // heuristic: center root fundamental solutions = unique solutions with first queen at center row; we cannot isolate easily here, fallback to expected dataset.
 ...
 {
 _solutionCount = fundamentalCountFromEngine; // large boards fundamental directly
 }
 }
 else
 {
 // Sequential path: use UniqueSolutionCounter then sample (pruned enumeration for sample only)
