...
internal sealed partial class BitmaskParallelEngine
{
    ...
    private static void ReportRootProgress(int done,int total,bool throttle,int bucketSize,ref int lastPercentReported,Action<double> report)
    {
        ...
        {
            int pctInt=(int)((double)done*100/total);int bucket=(pctInt/bucketSize)*bucketSize;int observed;while(bucket>(observed=Volatile.Read(ref lastPercentReported)))
            {if(Interlocked.CompareExchange(ref lastPercentReported,bucket,observed)==observed){report(bucket);break;}}
        }
    }
