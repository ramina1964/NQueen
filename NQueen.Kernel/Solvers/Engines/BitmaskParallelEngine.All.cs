    ...
    public void RunAll(AllRequest request)
    {
        ...
        while (rootStack.Count > 0)
        {
            ...
            while (avail != 0)
            {
                ...
                avail ^= bit; int row = BitOperations.TrailingZeroCount(bit);
                var rowsCopy = (int[])frame.Rows.Clone();
                rowsCopy[frame.Col] = row;
                ulong cols = frame.Cols | bit; ulong d1 = (frame.D1 | bit) << 1;
        ...
        foreach (var root in rootList)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                while (true)
                {
                    ...
                    rowsArr[col] = row; stackCols[col] = cols; stackD1[col] = d1; stackD2[col] = d2; stackRemaining[col] = remaining; cols |= bit; d1 = (d1 | bit) << 1; d2 = (d2 | bit) >> 1; col++; if (col == N) continue; remaining = ComputeAvail();
                }
