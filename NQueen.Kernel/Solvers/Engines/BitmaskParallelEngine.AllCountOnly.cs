...
{
    public static void RunAllCountOnly(AllCountOnlyRequest request)
    {
        int N = request.BoardSize;
        int splitDepth = request.RootSplitDepth < 1
            ? 1
            : request.RootSplitDepth;
        ...
        foreach (var root in rootList)
        {
            tasks.Add(Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {
                        if (col == N)
                        { localCount++; col--; if (col < startCol) break; Restore(col, out remaining); continue; }
                        if (remaining == 0) { col--; if (col < startCol) break; Restore(col, out remaining); continue; }
                        ulong bit = remaining & (ulong)-(long)remaining; remaining ^= bit; int row = BitOperations.TrailingZeroCount(bit); rowsArr[col] = row; stackCols[col] = cols; stackD1[col] = d1; stackD2[col] = d2; stackRemaining[col] = remaining; cols |= bit; d1 = (d1 | bit) << 1; d2 = (d2 | bit) >> 1; col++; if (col == N) continue; remaining = ComputeAvail();
                    }
                }
                finally
                {
                ...
                if (request.ReportProgress != null)
                {
                    ...
                    ReportRootProgress(done, totalRoots, throttle, bucketSize,
                        ref lastPercentReported, request.ReportProgress);
                }
